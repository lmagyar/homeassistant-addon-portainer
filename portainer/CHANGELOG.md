# Changelog

## vNext (forked)

- Update portainer/portainer to v2.39.6
- Update App base image to v21.0.2
- Properly handle failure during service startup

## 2.7.2 (forked)

- Update portainer/portainer to v2.39.5
- Update App base image to v21.0.1
- Graceful shutdown on SIGTERM caused by manual app stop

## 2.7.1 (forked)

- Update portainer/portainer to v2.39.4
- Update App base image to v21.0.0 (Update Alpine base image to v3.24.0)

## 2.7.0 (forked)

- Update portainer/portainer to v2.39.2
- Update Add-on base image to v20.1.1

## 2.6.4 (forked)

- Update portainer/portainer to v2.33.7
- Drop support for armv7 architecture
- Update App base image to v20 (drop armv7 support)

## 2.6.3 (forked)

- Update portainer/portainer to v2.33.6
- Remove deprecated codenotary fields

## 2.6.2 (forked)

- Fix for app base image and bashio log changes, causing healthcheck messages to be not logged

## 2.6.1 (forked)

- Update portainer/portainer to v2.33.3
- Update App base image to v18.2.1

## 2.6.0 (forked)

- Update portainer/portainer to v2.33.1
- Fix ingress by clearing conflicting security headers
- Update App base image to v18.1.0

## 2.5.7 (forked)

- Update portainer/portainer to v2.27.8
- Update App base image to v18.0.0 (Update Alpine base image to v3.22.0)

## 2.5.6 (forked)

- Update portainer/portainer to v2.27.7
- Update App base image to v17.2.5

## 2.5.5 (forked)

- Update portainer/portainer to v2.27.5

## 2.5.4 (forked)

- Update portainer/portainer to v2.27.4
- Update App base image to v17.2.4

## 2.5.3 (forked)

- Update portainer/portainer to v2.27.3

## 2.5.2 (forked)

- Update portainer/portainer to v2.27.2
- Update App base image to v17.2.2

## 2.5.1 (forked)

- Update portainer/portainer to v2.27.1

## 2.5.0 (forked)

- Update portainer/portainer to v2.27.0
- Configure log format for the app to be compatible with Portainer's format
- Update App base image to v17.2.1

## 2.4.12 (forked)

- Update App base image to v17.0.2

## 2.4.11 (forked)

- Update portainer/portainer to v2.21.5
- Update App base image to v17.0.1 (Update Alpine base image to v3.21.0)

## 2.4.10 (forked)

- Log HEALTHCHECK status and errors

## 2.4.9 (forked)

- Update portainer/portainer to v2.21.4
- Fix HEALTHCHECK support
- Update App base image to v16.3.6

## 2.4.8 (forked)

- Update portainer/portainer to v2.21.2
- Update App base image to v16.3.2

## 2.4.7 (forked)

- Update portainer/portainer to v2.21.1
- Update App base image to v16.3.1

## 2.4.6 (forked)

- Update portainer/portainer to v2.21.0
- Update App base image to v16.2.1

## 2.4.5 (forked)

- Update portainer/portainer to v2.20.3
- Update App base image to v16.0.0 (Update Alpine base image to v3.20.0)

## 2.4.4 (forked)

- Update portainer/portainer to v2.20.1
- Update App base image to v15.0.8

## 2.4.3 (forked)

- Update portainer/portainer to v2.20.0
- Update App base image to v15.0.7 (Update Alpine base image to v3.19.1)

## 2.4.2 (forked)

- Remove `log_level` fom the documentation, this is not used by the app
- Update portainer/portainer to v2.19.4
- Update App base image to v14.3.3

## 2.4.1 (forked)

- Update portainer/portainer to v2.19.3

## 2.4.0 (forked)

***BREAKING CHANGES:***
- Drop support for armhf & i386

Nonbreaking changes:
- Add HEALTHCHECK support
- Update portainer/portainer to v2.19.2
- Update App base image to v14.3.2

## 2.3.2 (forked)

- Update portainer/portainer to v2.19.1

## 2.3.1 (forked)

- Update portainer/portainer to v2.19.0
- Update App base image to v14.1.0 (Update Alpine base image to v3.18.3)

## 2.3.0 (forked)

- Update portainer/portainer to v2.18.4
- Update App base image to v14.0.2
- Sign app with Sigstore Cosign

## 2.2.3 (forked)

- Update Portainer to v2.18.3

## 2.2.2 (forked)

- Update App base image to v14 (Update Alpine base image to v3.18.0)

## 2.2.1 (forked)

- Update Portainer to v2.18.2

## 2.2.0 (forked)

- Update Portainer to v2.18.1
- Migrate app layout to S6 Overlay
- Update App base image to v13.2.2

## 2.1.18 (forked)

- Bump Portainer version from 2.17.0 to 2.17.1
- Bump base image from 13.1.1 to 13.1.2

## 2.1.17 (forked)

- Bump Portainer version from 2.16.2 to 2.17.0
- Bump base image from 13.0.1 to 13.1.1

## 2.1.16 (forked)

- Bump base image from 13.0.0 to 13.0.1

## 2.1.15 (forked)

- Bump base image from 12.2.7 to 13.0.0

## 2.1.14 (forked)

- Bump Portainer version from 2.16.1 to 2.16.2

## 2.1.13 (forked)

- Bump base image from 12.2.6 to 12.2.7

## 2.1.12 (forked)

- Bump Portainer version from 2.16.0 to 2.16.1

## 2.1.11 (forked)

- Bump base image from 12.2.5 to 12.2.6

## 2.1.10 (forked)

- Bump base image from 12.2.4 to 12.2.5

## 2.1.9 (forked)

- Bump Portainer version from 2.15.1 to 2.16.0

## 2.1.8 (forked)

- Bump Portainer version from 2.15.0 to 2.15.1

## 2.1.7 (forked)

- Bump base image from 12.2.3 to 12.2.4

## 2.1.6 (forked)

- Bump Portainer version from 2.14.2 to 2.15.0

## 2.1.5 (forked)

- Bump base image from 12.2.2 to 12.2.3

## 2.1.4 (forked)

- Bump Portainer version from 2.14.1 to 2.14.2
- Bump base image from 12.2.0 to 12.2.2

## 2.1.3 (forked)

- Bump Portainer version to 2.14.1

## 2.1.2 (forked)

- Bump Portainer version to 2.14.0

## 2.1.1 (forked)

- Fix script permissions

## 2.1.0 (forked)

- Bump base image from 11.1.2 to 12.2.0
- Sign app with Codenotary Community Attestation Service (CAS)

## 2.0.5 (forked)

- Bump Portainer version to 2.13.1

## 2.0.4 (forked)

- Bump Portainer version to 2.13.0
- Bump base image from 11.0.1 to 11.1.2

## 2.0.3 (forked)

- Exclude unnecessary files from backup

## 2.0.2 (forked)

- Bump Portainer version to 2.11.1

## 2.0.1 (forked)

- Bump Portainer version to 2.11.0
- Bump base image from 10.0.1 to 11.0.1

## 2.0.0.0 (forked)

- Make it available as a custom app

## 2.0.0

- Last known, deprecated community app

For previous changelog see the original app's [release history](https://github.com/hassio-addons/addon-portainer/releases).
